...
describe("UserList", () => {
  it("renders empty state when users array is empty", () => {
    render(<UserList users={[]} onEdit={vi.fn()} />);
    expect(screen.getByText(/no users found/i)).toBeInTheDocument();
  });

  it("renders a row for each user", () => {
    render(<UserList users={mockUsers} onEdit={vi.fn()} />);
  ...
  it("shows admin role badge for admin users", () => {
    render(<UserList users={[mockUsers[0]]} onEdit={vi.fn()} />);
  ...
  it("shows user role badge for regular users", () => {
    render(<UserList users={[mockUsers[1]]} onEdit={vi.fn()} />);
